                    ...
                    <div class="page-title-box">
                        <ol class="breadcrumb float-left">
                            ...
                            @if($transaction->status==1 && $transaction->payment==3)
                                ...
                                <li class="breadcrumb-item"><a href="{!!url('/admin/new-order-cod')!!}">Đơn Hàng Mới</a>
                                </li>
                            @elseif($transaction->status==1 && $transaction->payment!=3)
                                <li class="breadcrumb-item">Đơn Hàng Trả Trước</li>
                                <li class="breadcrumb-item"><a href="{!!url('/admin/new-order')!!}">Đơn Hàng Mới</a>
                                </li>
                            @elseif($transaction->status!=1 && $transaction->payment==3)
                                <li class="breadcrumb-item">Đơn Hàng Trả Sau</li>
                                <li class="breadcrumb-item"><a href="{!!url('/admin/view-order-cod')!!}">Danh Sách
                                        Đơn</a></li>
                            @else
                                <li class="breadcrumb-item">Đơn Hàng Trả Trước</li>
                                <li class="breadcrumb-item"><a href="{!!url('/admin/view-order')!!}">Danh Sách Đơn</a>
                                </li>
                            @endif
                    ...
                    <div class="card-box">
                        ...
                        <form action="{!!url('admin/edit-order')!!}" method="post" id="main-form">
                            ...
                            <div class="form-group col-lg-9 wrapper detailOrder">
                                ...
                                <a href="{!!url('/admin/invoice',$transaction->id)!!}"
                                   class="float-right btn btn-info btn-sm text-uppercase">in hóa đơn</a>
                                ...
                                @foreach($transaction->products as $item)
                                    <div class="mt-4 mt-sm-3 oneProduct clearfix">
                                        ...
                                        @endphp
                                        <span class="col-sm-3 float-left mt-1 mt-sm-0">Qty:<input
                                                    @if($transaction->status<>1)readonly @endif type="number"
                                                    name="quantity[]" class="quantity" value="{!!$quantity!!}" min="1"
                            ...
                            <div class="form-group col-lg-9 wrapper total">
                                ...
                                <div class="form-group">
                                    ...
                                    <input @if($transaction->status<>1)readonly @endif type="number"
                                           id="amount_shipping" name="amount_shipping"
                                           value="{!!$transaction->amount_shipping!!}">
                                </div>
                                <div class="form-group">
                                ...
                                @if($transaction->payment==2)
                                    <div class="pay_status">
                                        @if($transaction->pay_status==1)
                                            <div class="form-group label"><label for="">Thanh toán:</label>Đã thanh toán
                                            </div>
                                        @else
                                            <input class="form-check-inline" type="checkbox" name="pay_status"
                                                   value="1">Đã thanh toán
                                        @endif
                                    </div>
                                @endif
                                @if($transaction->status==1)
                                    <button class="btn btn-success mt-3" name="btnReload" type="submit" form="main-form"
                                            value="btnReload">
                                        Làm mới
                                    </button>
                                @endif
                            ...
                            <div class="form-group col-lg-9 wrapper infoUser">
                                ...
                                <div class="form-group label"><label for="">Tên:</label>{!!$transaction->user->name!!}
                                </div>
                                <div class="form-group label"><label
                                            for="">Email:</label>{!!$transaction->user->email!!}</div>
                                <div class="form-group label"><label for="">Số điện
                                        thoại:</label>{!!$transaction->user->phone!!}</div>
                                <div class="form-group label"><label for="">Địa chỉ khách
                                        hàng:</label>{!!$transaction->user->address!!}</div>
                                <div class="label"><label for="">Địa chỉ giao
                                        hàng:</label>@if(isset($transaction->user->other_address)){!!$transaction->user->other_address!!}@else{!!$transaction->user->address!!}@endif
                                </div>
                                <textarea class="form-group mt-2" readonly cols="40" rows="5"
                                          placeholder="Ghi chú của khách hàng">{!!$transaction->note!!}</textarea>
                            </div>
                                ...
                                <div class="form-group col-lg-9">
                                    @if($transaction->status<4)
                                        <button class="btn btn-gradient waves-effect waves-light" type="submit"
                                                name="btnApproval" form="main-form" value="btnApproval">
                                            @if($transaction->status==1)
                                                Duyệt Đơn Hàng
                                            @elseif($transaction->status==2)
                                        ...
                                        <a href="{!!url('/admin/cancellation',$transaction->id)!!}"
